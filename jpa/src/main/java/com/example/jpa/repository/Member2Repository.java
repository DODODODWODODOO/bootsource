...
public interface Member2Repository extends JpaRepository<Member2, Long>, QuerydslPredicateExecutor<Member2> {
    ...
    @Query("SELECT m.userName, m.age FROM Member2 m") // m.~~~ 개별로 지정 가능
    ...
    // com.example.jpa.entity.Member2]
    // => List<Object[]> findByMembers2(); 오브젝트 배열로 변경
    ...
    List<Member2> findByTeamEqualList(Team2 team2);
    ...
    // 내부 조인 : ON 구문 사용X jpql_team t1_0 on t1_0.team_id=m1_0.team2_team_id where
    // t1_0.team_name=?
