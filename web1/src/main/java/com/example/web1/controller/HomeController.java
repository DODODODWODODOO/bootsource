...
public class HomeController {
    ...
    // rttr.addAttribute("이름", 데이터(값)); => 파라메터로 전달
    // rttr.addFlashAttribute("이름", 값); => Session 을 이용(임시)해 값을 저장
    ...
    public String ex3(RedirectAttributes rttr) {
        ...
        return "redirect:/sample/basic"; // 경로지정(다른 컨트롤러에 있는 경로도 포함)
    }

    // IllegalStateException: Ambiguous mapping => 전체 컨트롤러에서 동일한 매핑 방식과 경로 지정
    // @GetMapping("/ex3")
    // public void ex4() {
    // log.info("/ex3 요청");
    // }
