...
public class MartRepositoryTest {
    ...
    @Autowired
    private DeilveryRepository deilveryRepository;

    @Test
    public void insertTest() {
        // 맴버 3명 추가
        memberRepository.save(Member.builder().name("user1").city("서울시").street("중구").zipcode("151569").build());
        memberRepository.save(Member.builder().name("user2").city("경기도").street("수원시").zipcode("654834").build());
        memberRepository.save(Member.builder().name("user3").city("강원도").street("강릉").zipcode("151569").build());
    ...
    public void readTest() {
        ...
        // order=Order(id=1, member=Member(id=1, name=user1, zipcode=151569, city=서울시,
        // street=중구), orderDate=2024-04-05T15:04:26.500626, orderStatus=ORDER),
        // orderPrice=26000, count=2)
        // 객체 그래프 탐색 => N:1 관계에서 FetchType.EAGER 이기 때문에
        orderItemRepository.findAll().forEach(entity -> {
            System.out.println(entity);

            System.out.println("상품 정보 " + entity.getItem());
    ...
    public void readTest3() {
        ...
        System.out.println("주문 내역 " + member.getOrders());
    ...
    public void updateTest() {
        // 맴버 주소 수정
        Member member = memberRepository.findById(2L).get(); // 불러오기
        member.setCity("대구"); // 수정
    ...
    public void deleteTest() {
        ...
        // orderRepository.deleteById(1L); ==> 주문아이템이 존재하기 때문에 에러발생
    ...
    public void orderInsertDeliveryTest() {
        ...
        deilveryRepository.save(delivery);
    ...
    public void deliveryOrderGet() {
        ...
        Delivery delivery = deilveryRepository.findById(1L).get();
